  ...
  getProductById: async (productId) => {
    try {
      const response = await fetch(`${API_BASE_URL}/product/${productId}`, {
          headers: {
              // API chi tiết sản phẩm thường yêu cầu token
...
export const categoryService = {
  getAllCategories: async (page = 1, size = 20) => {
    try {
      const response = await fetch(`${API_BASE_URL}/category?page=${page}&size=${size}`);
      const result = await response.json();
      return result.success ? result.data : [];
    } catch (error) {
...
export const cartService = {
  addToCart: async (productDetailId, quantity) => {
    try {
      ...
      if (!token || token === "MOCK_TOKEN") return { success: false, message: 'Chưa đăng nhập. Vui lòng đăng nhập để thêm sản phẩm.' };
      ...
      return {
        ...
        message: result.message || result.error || 'Đã thêm vào giỏ hàng thành công'
      };
    } catch (error) {
